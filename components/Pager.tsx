...
export default function Pager({ currentPage, perPage, total, urlRoot }: { currentPage: number, perPage: number, total: number, urlRoot: string }) {
        ...
        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            ...
            <div>
                <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                    <a
                        href={urlRoot + '?page=' + (currentPage-1)}
                    ...
                    {rangeArr?.map(pageNumber => {
                        if(pageNumber === DOTS) {
                            return (
                                <span key={'page' + DOTS} className="relative inline-flex items-center border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700">
                                    ...
                                </span>
                            )
